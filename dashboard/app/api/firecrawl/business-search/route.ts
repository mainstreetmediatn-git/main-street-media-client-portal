...
export async function POST(request: Request) {
  ...
  try {
    ...
    const query = requireString(body?.query, "business search query");
    const location = typeof body?.location === "string" ? body.location.trim().slice(0, 120) : "";
    const data = await runFirecrawl("business-search", () =>
      getFirecrawlClient().search(location ? `${query} ${location}` : query, { sources: ["web"], limit: 10, timeout: 15_000 })
    );
